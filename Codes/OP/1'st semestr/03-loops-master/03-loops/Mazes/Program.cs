...
namespace Mazes
{
	internal static class Program
	{
		...
		{
			yield return new MazeTestCase("empty", MazeTasks.MoveOutFromEmptyMaze);
			...
			yield return new MazeTestCase("pyramid", MazeTasks.MoveOutFromPyramidMaze);
			yield return new MazeTestCase("snail1", MazeTasks.MoveOutFromSpiralMaze);
			yield return new MazeTestCase("snail2", MazeTasks.MoveOutFromSpiralMaze);


//			Hey! You've just found the secret levels! You can earn points for them too.
	...
	internal class MazeTestCase : TestCase
	{
		...
		protected override void InternalVisualize(TestCaseUI ui)
		{
			...
			ui.Circle(Conv(robot.Pos.X), Conv(robot.Pos.Y), cellSize / 3.0, actualAnswerPen);
		...
		protected override bool InternalRun()
		{
			...
			return robot.Pos.Equals(maze.Exit);
		}
	}
}